...
static void client_send_message(char const *message) {
  ...
  lcd_print(1, 8, COLOR_BLACK,"Tx:sending");
  ...
  lcd_print(1, 8, COLOR_BLACK,"Tx:sent %s", error_2_string(ret_code));
...
int send_message(esp_sleep_source_t trigger) {
  ...
  cJSON_AddItemToObject(json_root, "Location", cJSON_CreateString("25.071884,121.576716"));
  // data element
